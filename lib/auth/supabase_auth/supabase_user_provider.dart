...
class LlibrejantSupabaseUser extends BaseAuthUser {
  LlibrejantSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> llibrejantSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LlibrejantSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
